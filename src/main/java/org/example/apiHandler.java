...
public class apiHandler {
    ...
    public static void foo() {
        try (
            ...
            HttpEntity entity = resp.getEntity();
            String res = convertStreamToString(entity.getContent());
            String respBodyString = EntityUtils.toString(entity);
            JSONObject obj = new JSONObject(respBodyString.substring(1));
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    public static void foo_2() {
        ...
        HttpGet request = new HttpGet("http://jsonplaceholder.typicode.com/posts?_limit=10");
        HttpResponse resp;
        String res = null;
        ...
        try {
            ...
            if (entity != null) {
                ...
                List<JSONObject> list_of_obj = new ArrayList<>();
                String[] list_of_json = res.split(",END_OF_ONE_JSON");

                for (int i = 0; i < list_of_json.length; i++) {
                    list_of_obj.add(new JSONObject(list_of_json[i]));
                }

                for (int i = 0; i < list_of_json.length; i++) {
                    System.out.println(list_of_obj.get(i));
                }
            }
